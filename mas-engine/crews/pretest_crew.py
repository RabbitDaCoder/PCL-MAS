...
# clearly-labeled mock questions when the Instructor Agent has no configured LLM — same contract
...
class AssessmentGenerationError(Exception):
    """Raised when the Instructor Agent's output can't be parsed into valid questions after a
...
def _mock_questions(topics: list[str]) -> list[AssessmentQuestion]:
    ...
    return [
        AssessmentQuestion(
            topic=topic,
            question=f"[MOCK — Instructor agent has no API key configured] Sample question about {topic}?",
